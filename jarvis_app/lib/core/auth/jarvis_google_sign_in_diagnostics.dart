...
abstract final class JarvisGoogleSignInDiagnostics {
  ...
  /// Only stable native fields are safe to share; never output idToken
  /// even if a native plugin accidentally returns it alongside the status.
  static String nativeResultDiagnostic(
    ...
    final String safeStage =
        stage.replaceAll(RegExp(r'[^a-zA-Z0-9_-]'), '_');
    final String reported = result?['status']?.toString() ?? 'no_result';
    ...
    final String safeResult =
        allowed.contains(reported) ? reported : 'unknown';
    final String rawCode = result?['googleStatus']?.toString() ?? '';
    final int? parsedCode = int.tryParse(rawCode);
    final String safeCode =
        parsedCode != null && parsedCode >= 0 && parsedCode <= 99999
            ? parsedCode.toString()
            : 'not reported';
    return 'Stage: $safeStage\n'
        'Result: $safeResult\n'
