...
class JarvisWatchPhoneStatus {
  const JarvisWatchPhoneStatus({
...
class JarvisWatchPhoneMessage {
  ...
  factory JarvisWatchPhoneMessage.fromJson(
    Map<String, dynamic> json,
  ) {
    return JarvisWatchPhoneMessage(
      id: json['id']?.toString() ?? '',
      callerName:
          json['callerName']?.toString() ?? '',
      callerPhone:
          json['callerPhone']?.toString() ?? '',
      callbackNumber:
          json['callbackNumber']?.toString() ?? '',
...
class JarvisApiService {
  ...
  Future<JarvisWatchPhoneStatus>
      watchPhoneStatus() async {
    final response = await _client.get(
      Uri.parse(
        '${_config.watchBridgeBaseUrl}/main/phone/status',
      ),
      headers: _headers,
    );
    ...
    if (response.statusCode != 200) {
      final String detail = decoded is Map
          ? decoded['detail']?.toString() ??
              'Watch Bridge phone status failed.'
          : 'Watch Bridge phone status failed.';
    ...
    if (decoded is! Map) {
      throw const FormatException(
        'Invalid Watch Bridge phone status response.',
      );
    }
    ...
    return JarvisWatchPhoneStatus(
      active: data['active'] == true,
      phoneNumber:
          data['phoneNumber']?.toString() ?? '',
      assistantName:
          data['assistantName']?.toString() ??
              'JARVIS Phone Receptionist v2',
      provider:
          data['provider']?.toString() ?? 'vapi',
    );
  }

  Future<List<JarvisWatchPhoneMessage>>
      watchPhoneMessages() async {
    final response = await _client.get(
      Uri.parse(
        '${_config.watchBridgeBaseUrl}/main/phone/messages',
      ),
      headers: _headers,
    );
    ...
    if (response.statusCode != 200) {
      final String detail = decoded is Map
          ? decoded['detail']?.toString() ??
              'Watch Bridge phone messages failed.'
          : 'Watch Bridge phone messages failed.';
      throw StateError(detail);
    }
    ...
    if (decoded is! Map) {
      throw const FormatException(
        'Invalid Watch Bridge phone messages response.',
      );
    }

    final Object? rawMessages = decoded['messages'];
    if (rawMessages is! List) {
      return const <JarvisWatchPhoneMessage>[];
    }
    ...
    return rawMessages
        ...
        .map(
          (Map raw) =>
              JarvisWatchPhoneMessage.fromJson(
